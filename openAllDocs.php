...
if(isset($cinfo) && ($cinfo[0] == "Status: OK" && $cinfo[1] == "MSG: Session Created"))
{
	...
	$curl_handler=curl_init('https://cs4743.professorvaladez.com/api/request_all_documents');
	$result;
	...
	if(isset($result))
	{
		...
		if($cinfo[0] == "Status: OK" )
		{
			...
			$fp=fopen("/var/www/toProcess/allDocName.txt", "a");
			...
			foreach($payload as $key=>$value)
			{
				...
				try {
					...
					if (fwrite($fp, $value . "\n") == false )
					{
						throw new Exception("Filename $value unable to be written to /var/www/toProcess/allDocName.txt");
					}
				}
				catch(Exception $e) {
